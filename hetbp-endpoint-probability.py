...
This script is copied from a similarly named script,
and its hetbp name means "heterogeneous blinking process."
Its purpose is to look at what happens when 
each blink thread has its own on and off rates.
"""
...
def create_compound_rate_matrix(
        part_to_rate_off, part_to_rate_on, partition, distn, dg):
    """
    ...
    @param part_to_rate_off: rate from the 'on' state to the 'off' state
    @param part_to_rate_on: rate from the 'off' state to the 'on' state
    ...
    for i in range(ncompound):
        ...
        for j in range(ncompound):
            ...
            if blink_hdist == 1:
                ...
                # set the compound state transition rate
                if diff == 1:
                    rate = part_to_rate_on[blink_part]
                elif diff == -1:
                    if blink_part == part_i:
                        rate = 0
                    else:
                        rate = part_to_rate_off[blink_part]
                else:
                    raise Exception
                pre_Q[i, j] = rate
...
def main(args):
    ...
    # Read the blink rates from a database file.
    conn = sqlite3.connect(args.blink_rates)
    cursor = conn.cursor()
    part_to_rate_off = dict(cursor.execute(
        'select part, rate_off from rates'))
    part_to_rate_on = dict(cursor.execute(
        'select part, rate_on from rates'))
    conn.close()
    ...
    for part in parts:
        ...
        if (rate_on + rate_off) in (0, float('Inf')):
    ...
    compound_to_primary, compound_to_blink, Q = create_compound_rate_matrix(
            part_to_rate_off, part_to_rate_on,
            partition, distn, dg)
    ...
    for i in range(ncompound):
        ...
        prob = adjusted_distn[pri]
        for blink_part, blink_state in enumerate(blink):

            # get the distribution from the rates from the part
            rate_on = part_to_rate_on[blink_part]
            rate_off = part_to_rate_off[blink_part]
            blink_distn = np.array([
                rate_off / (rate_on + rate_off),
                rate_on / (rate_on + rate_off),
                ], dtype=float)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--blink-rates', default='blink.rates.db',
            help='blink on and off rates per part as an sqlite3 database file')
